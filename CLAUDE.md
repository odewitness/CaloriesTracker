...
- `ErrorBoundary` (`src/components/ErrorBoundary.jsx`) à 3 niveaux dans `App.jsx` : racine (dernier filet), autour du contenu d'onglet (`label="cet écran"`, `resetKey={activePath}`), autour des modales plein écran (`label="cette fenêtre"`, `overlay`, `onClose={closeOverlay}`, `resetKey={location.pathname}`). Un plantage de rendu affiche un panneau « Réessayer / Recharger » + détail technique dépliable, sans tuer le reste de l'app ; la navigation (`resetKey`) sort de l'état d'erreur automatiquement. Les erreurs asynchrones (hors rendu) ne sont pas capturées par un ErrorBoundary.
- **Toute nouvelle carte/section ajoutée à la page du jour (`TodayPage`) doit passer par le mécanisme de réordonnancement** (`settings.ordre_sections_jour`, voir `src/lib/todaySections.js`) plutôt qu'être rendue à une position fixe dans le JSX : ajouter la clé à `TODAY_SECTION_KEYS`/`TODAY_SECTION_LABELS`, puis une entrée dans l'objet `sectionNodes` de `TodayPage.jsx` (pattern déjà suivi par `bilan`, `sport`, `eau`, `transit`, etc.) — sinon l'utilisatrice ne peut pas la déplacer depuis Profil > Page du jour. Si la carte est réservée à un seul compte (comme le tracker de transit, `STOOL_TRACKER_USER_ID`), filtrer aussi sa clé de la liste passée à `TodaySection` dans `ProfilePage.jsx` pour l'autre compte (sinon son nom apparaît dans les réglages d'un compte qui n'y a pas accès).
