...
import {
  cycleInfo, phasesForRange, PHASES, formatPredictionWindow, daysBetween,
} from '../../lib/cycle'

// ─────────────────────────────────────────────────────────────────────────────
// Écran de détail « Cycle & alimentation ».
// Palier 1 : activer/désactiver, saisir à la main les 1ers jours de règles
// (dont plusieurs cycles passés), voir la phase courante, régler les
// longueurs. Aucun changement des cibles caloriques (ce sera le Palier 3).
// Voir docs/cycle-menstruel.md.
// ─────────────────────────────────────────────────────────────────────────────
export default function CycleSection({ cycle, onPatch, onBack }) {
  const cfg = cycle || {}
  const { dates, loading, toggleStart, removeStart } = useCycle()
  const [monthAnchor, setMonthAnchor] = useState(new Date())

  const today = todayStr()
  const info = useMemo(() => cycleInfo(today, dates, cfg), [today, dates, cfg])
  ...
  const cycleByDate = useMemo(() => {
    ...
    return phasesForRange(fmt(cells[0].date), fmt(cells[cells.length - 1].date), dates, cfg)
  }, [monthAnchor, dates, cfg])

  const selectedDates = useMemo(() => new Set(dates), [dates])

  // Débuts de règles, du plus récent au plus ancien, avec l'écart au précédent.
  const rows = useMemo(() => {
    const asc = [...dates].sort()
    return asc.map((d, i) => ({
      date: d,
      gap: i > 0 ? daysBetween(asc[i - 1], d) : null,
    })).reverse()
  }, [dates])
    ...
    <SectionScreen title="Cycle & alimentation" onBack={onBack}>
      ...
      {!enabled && (
        <div style={{ fontSize: 12.5, color: 'var(--text-muted)', lineHeight: 1.6 }}>
          Tu notes toi-même le 1er jour de tes règles (l'app ne se synchronise avec
      ...
      {enabled && (
        <>
          ...
          {/* Saisie / historique */}
          <div className="section-title" style={{ marginTop: 8 }}>Tes débuts de règles</div>
          <div style={{ fontSize: 11.5, color: 'var(--text-hint)', lineHeight: 1.5, marginBottom: 8 }}>
            Touche un jour pour marquer (ou enlever) un 1er jour de règles. Remonte
          ...
          {!loading && rows.length > 0 && (
            <div className="card" style={{ marginBottom: 16, overflow: 'hidden' }}>
              {rows.map(({ date, gap }) => (
                <div key={date} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 16px', borderBottom: '0.5px solid var(--border)' }}>
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--coral)', flexShrink: 0 }} />
                  <div style={{ flex: 1, fontSize: 13.5 }}>{labelDate(date)}</div>
                  {gap != null && (
                    <span style={{ fontSize: 11.5, color: 'var(--text-hint)' }}>cycle de {gap} j</span>
                  )}
                  <button onClick={() => removeStart(date)} aria-label="Supprimer" style={{ padding: 4, color: 'var(--text-hint)', background: 'none' }}>
                    <Trash2 size={15} />
                  </button>
                </div>
              ))}
...
function PhaseSummary({ info, sousContraception }) {
  ...
  if (info.phase === 'inconnue') {
    line2 = info.reason === 'no-data'
      ? 'Saisis au moins un 1er jour de règles ci-dessous.'
      : info.overdueBy > 0
  ...
  return (
    <div className="card" style={{ padding: '12px 16px', marginBottom: 16, borderLeft: `3px solid ${phase.color}` }}>
      ...
      {info.observedCycleLen != null && (
        <div style={{ fontSize: 11, color: 'var(--text-hint)', marginTop: 6 }}>
          Cycle observé : {info.observedCycleLen} j (sur {info.nCycles} cycle{info.nCycles > 1 ? 's' : ''} enregistré{info.nCycles > 1 ? 's' : ''})
        </div>
      )}
    </div>
  )
