...
// ─────────────────────────────────────────────────────────────────────────────
// FoodSortModal — feuille « Trier & filtrer » à onglets (Filtrer / Trier), avec
// un résumé de chips supprimables sous le titre. Même design que SortModal
// (recettes), sans le sélecteur de base de calcul ni les filtres de durée (pas
// pertinents pour un aliment).
// ─────────────────────────────────────────────────────────────────────────────
export default function FoodSortModal({ value, onChange, onClose }) {
  ...
  const sortIsCustom = primaryField !== 'nom' || primaryDir !== 'asc' || !!secondaryField
  ...
  const activeChips = [
    ...
    ...(primaryField !== 'nom' || primaryDir !== 'asc' ? [{
      id: 'sort-primary',
      label: `Trier : ${describeSortField({ field: primaryField, dir: primaryDir })}`,
      onRemove: () => { setPrimaryField('nom'); setPrimaryDir('asc') },
    }] : []),
    ...(secondaryField ? [{
      id: 'sort-secondary',
      label: `Puis : ${describeSortField({ field: secondaryField, dir: secondaryDir })}`,
