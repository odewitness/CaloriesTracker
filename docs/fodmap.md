...
| **1 — Fiche aliment** | 🔧 Branche `feature/fodmap-fiche-aliment` (2026-09-26) : `src/lib/fodmap.js` (calcul), `src/hooks/useFodmapProfile.js` (relit la ligne Ciqual à jour), `src/components/FodmapPanel.jsx` (bloc dans l'ajout d'aliment, la fiche d'une entrée du journal et la fiche de l'explorateur ; masqué pour recettes et compléments), Profil > FODMAP (`FodmapSection.jsx`). Mots-clés du nom pour les aliments hors table (« probablement élevé »). **À exécuter avant le merge : `supabase/sql/fodmap_settings_setup.sql`** | `settings.fodmap` | M |
