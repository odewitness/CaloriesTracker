...
- **Repas** : cumul par famille des entrées du même `meal` (§3.3). Depuis
  le Palier 3, une recette du journal compte pour ses ingrédients.
...
- **Recette** (Palier 3, `recipeFodmapItems` dans
  `src/hooks/useFodmapProfile.js`) : une portion de P g = chaque ingrédient
  à sa quantité crue × P ÷ poids de référence (`poids_cuit_g`, sinon
  `poids_cru_g`, sinon somme des ingrédients — même référence que l'ajout au
  journal), puis cumul comme un repas (`evaluateMeal`). Une entrée de journal
  avec `ingredients_detail` (grammages corrigés à l'ajout) garde les
  proportions de cet instantané, appariées par nom. Portion sûre : charges
  calculées pour 100 g, proportionnelles à la portion (`mealSafePortion`).
  Affiché : niveau par famille en « × le seuil », ingrédients qui pèsent le
  plus, et alternatives couramment citées (`fodmapSubstitution`) pour les
  ingrédients des familles modérées ou élevées.
...
| **2 — Repas & journée** | ✅ Mergé sur main le 2026-09-26 : `evaluateMeal` (cumul par repas, §3.3), `useDayFodmap` (une requête Ciqual groupée par jour), ligne « FODMAP du repas » sous l'en-tête de chaque repas, pastille sur les aliments modérés/élevés/probablement élevés du journal (`FodmapPill.jsx`), carte « FODMAP » de la page du jour (`FodmapDayCard.jsx`, clé `fodmap` de `todaySections.js`, affichée seulement si activé). Recettes non comptées | Non | M |
| **3 — Recettes & explorateur** | ✅ Mergé sur main le 2026-09-26 : carte FODMAP des recettes (fiche recette onglet Nutriments, ajout d'une recette, fiche d'une recette du journal) avec ingrédients responsables et alternatives ; recettes comptées dans le cumul des repas ; explorateur : section « FODMAP à la portion » dans les filtres (faibles en FODMAP / par famille, à la portion usuelle, familles inconnues écartées) et pastille sur les aliments modérés/élevés. **3b** ✅ mergé le 2026-09-26 (colonne créée en prod) : réglage par famille « Auto / Absent / Présent » dans le formulaire d'aliment perso (`aliments_custom.fodmap`, `supabase/sql/fodmap_aliments_custom_setup.sql`, prioritaire sur le calcul, relu depuis le journal et les recettes) ; Open Food Facts : fructose, glucose, lactose et polyols repris quand le produit les déclare (null sinon, jamais 0) | `aliments_custom.fodmap` (3b) | M |
| **4 — Transit** | ✅ Mergé sur main le 2026-09-26 (table créée en prod) : table `symptomes_digestifs` (une ligne = un épisode : symptômes multi-sélection ballonnement/douleur/gaz/urgence, intensité 1-3, heure, note ; `supabase/sql/symptomes_digestifs_setup.sql`) ; feuille du tracker avec bascule « Passage / Symptôme sans passage », symptômes listés dans la carte Transit ; onglet Digestion : bloc « Symptômes sans passage » et cartes « FODMAP & transit » (toutes familles puis par famille : jours avec au moins un repas modéré/élevé/probablement élevé vs autres jours, type Bristol et part de jours avec symptômes à J et J+1, part de jours en phase lutéale en regard ; `usePeriodFodmap`, `fodmapTransitStats` ; symptômes = épisodes + passages avec douleur/ballonnement ; comparaison symptômes seulement si au moins un symptôme noté ; ≥ 2 jours de chaque côté ; masqué en vue Année et si l'affichage FODMAP est désactivé) | `symptomes_digestifs` | M |
| **5 — Extensions** | 🚧 Branche `feature/fodmap-scanner-courses` : produits Open Food Facts — `analyzeIngredients` repère dans la liste d'ingrédients et les additifs les sources de FODMAP (polyols E420/E421/E953/E965/E966/E967, inuline/FOS/chicorée, ail/oignon, blé/seigle/orge hors amidon/sirop de glucose/gluten, légumineuses, lait/lactose hors lait végétal/beurre de cacao/protéines de lait, fructose/miel/agave/jus de pomme-poire) ; principal = 3 premiers ingrédients, ≥ 5 % ou additif/inuline → « probablement élevé » si la famille n'est pas chiffrée ; affiché dans la carte FODMAP (« Repéré dans les ingrédients »), non enregistré au journal. Liste de courses : pastille « FODMAP au-delà de ~X g » (portion sûre < 100 g) ou « FODMAP probables » (nom), articles libres jugés sur leur nom (`useShoppingFodmap`, `shoppingFodmapHint`). **Planificateur : non fait**, toujours à rediscuter (frontière avec l'accompagnement du régime, hors périmètre §1) | Non | M |
